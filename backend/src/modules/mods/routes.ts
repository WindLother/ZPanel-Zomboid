...
export async function modsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/mods/lookup', { preHandler: requireRole('admin') }, async (req) => {
  ...
  app.post('/api/mods', { preHandler: requireRole('admin') }, async (req) => {
    const body = z
  ...
  app.delete('/api/mods/:workshopId', { preHandler: requireRole('admin') }, async (req) => {
  ...
  app.delete('/api/mods/standalone/:modId', { preHandler: requireRole('admin') }, async (req) => {
  ...
  app.post('/api/mods/:workshopId/toggle', { preHandler: requireRole('admin') }, async (req) => {
    const { workshopId } = workshopIdParam.parse(req.params);
